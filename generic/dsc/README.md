...
## Example
```bash
gst-launch-1.0 videotestsrc pattern=ball num-buffers=120 ! "video/x-raw,framerate=30/1" ! videoconvert ! x264enc ! dscsigner private-key-path= ./example_ca.key ! dscverifier public-key-path= ./example_ca.pub !  h264parse ! avdec_h264 ! videoconvert ! autovideosink
```
